...
TARGET_COLOR_BLUE = "#00c1cd"
TARGET_COLOR_RED = {"#fe2c55", "#ff0000", "#fc7265"}
...
def process_capcut_json(json_path):
    ...
    def extract_markers_for_color(color_check):
        """Extract project-timeline positions for markers matching color_check (str or set)."""
        result_map = {}
        for mid, mat in materials.items():
            marks = mat.get("mark_items") or mat.get("markItems") or mat.get("time_marks") or []
            for mk in marks:
                color = (mk.get("color") or "").lower()
                if isinstance(color_check, set):
                ...
                for s in segs:
                    ...
                    if proj_dur and not (0 <= proj_us <= int(proj_dur)):
                        continue
                    prev = result_map.get(title)
                    if prev is None or proj_us < prev:
                        result_map[title] = proj_us
        return result_map

    # Blue markers → Timecodes
    blue_map = extract_markers_for_color(TARGET_COLOR_BLUE)
    output = [(0, "Einleitung")]  # Always add introduction
    for t, us in blue_map.items():
        output.append((us, t))
    ...
    # Red markers → Linked videos
    red_map = extract_markers_for_color(TARGET_COLOR_RED)
    if red_map:
        red_output = sorted(red_map.items(), key=lambda x: x[1])
        out_lines.append("")
        out_lines.append("Linked videos:")
        for title, us in red_output:
            out_lines.append(f"{fmt(us)} - {title}")
